...
resource "aws_cloudfront_distribution" "example_cf" {
  ...
  origin {
    domain_name = aws_route53_record.example_route53_record.fqdn
    origin_id   = aws_lb.example_alb.name
  ...
  origin {
    domain_name = aws_s3_bucket.example_static_bucket.bucket_regional_domain_name
    origin_id   = aws_s3_bucket.example_static_bucket.id
  ...
  default_cache_behavior {
    ...
    target_origin_id       = aws_lb.example_alb.name
  ...
  ordered_cache_behavior {
    ...
    allowed_methods  = ["GET", "HEAD"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = aws_s3_bucket.example_static_bucket.id
  ...
  aliases = ["www.example.${var.domain}"]
...
resource "aws_route53_record" "example_route53_cloudfront" {
  zone_id = data.aws_route53_zone.example_route53_zone.id
  name    = "www.example.${var.domain}"
  type    = "A"

  alias {
    name                   = aws_cloudfront_distribution.example_cf.domain_name
    zone_id                = aws_cloudfront_distribution.example_cf.hosted_zone_id
